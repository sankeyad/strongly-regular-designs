...
#	
#
# differs from SRD-onef.py in that it reads from two separate files of srg parameters.

# Important: Use SRD-onef.py if you want to compare all pairs of parameters in a single range.
# This code is used when n1 is 101 to 150, and n2 is 251-300, for example. 
#
# Usage: SRD.py <first file> <second file> <output file>
# The input files should be tab-separated text files with one SRG parameter set in the form 'n k lambda mu' per line, e.g.:
# 204	63	22	18       
# 204	28	2	4
# 205	96	50	40
# 205	68	15	26
...
def gettable(infile):   # reads list of srgs from text file

	graphs = open(infile)
	lines = []
	# extract a line from the text file of srgs
	lines.append(graphs.readline().split('\t'))	 # read a string, parse it to get list
...
#read each file only once: get lines1, lines2
lines1 = gettable(infile1)
lines2 = gettable(infile2)

g1 = [0,0,0,0]
g2 = [0,0,0,0]
lines = []
# extract a pair of lines from the text file of srgs

count1 = len(lines1)-1
count2 = len(lines2)-1

for i in range(0,count1):
	for j in range(0, count2):   # two separate lists so looping through all of both
		line1 = lines1[i]  # this reads in text, separated by spaces, into list
		line2 = lines2[j]
...
# Loops through both input files
